...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'NVn7FzP6qwhSfK0',
      'username':'Elza.Bosco13',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Elza.Bosco13' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'Q_xZj9RW1rtapv3',
      'username':'Alf_Purdy60',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Alf_Purdy60' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
